$(document).ready(function(){
	...
	function inicializarTablas() {
		tableInscripciones = $('#example').dataTable();
		tableAnual = $('#example2').dataTable();
	}
	...
	function clearTables() {
		...
		//primero inicializamos las tablas
		inicializarTablas();
		...
		tableInscripciones.fnClearTable();
		tableAnual.fnClearTable();
	...
	function callback_ok( result ) {
		...
		if ( result['response']!=0){
			...
			tableInscripciones.fnReloadAjax( {						    	
				"aaData": result['data'],
				"aoColumns": [
				              { "data": "id" },
				              { "data": "firstaccess" },
