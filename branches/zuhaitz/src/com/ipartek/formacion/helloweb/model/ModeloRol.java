...
import com.ipartek.formacion.helloweb.interfaces.IModeloRol;
...
public class ModeloRol implements IModeloRol {
	 ...
	 */
	private static final String ROLE_EXC_ID_INCORRECT = "Identificador incorrecto %d";

	/**
	 * Texto excepción para parámetro a null.
	 */
	private static final String ROLE_EXC_OBJ_NULL = "El objeto pasado no puede ser null";

	/**
	 * Texto excepción para objeto no encontrado.
	 */
	private static final String ROLE_EXC_OBJ_NOT_FOUND = "No se ha encontrado el objeto %d";
	 ...
	 */
	public static ArrayList<Role> getListRoles() {
		return roles;
	}
	...
	public final Role getByID(final int identi) throws java.lang.Exception {
		...
		if (identi <= Role.ID_NULL) {
			throw new Exception(ROLE_EXC_UPDATE + String.format(ROLE_EXC_ID_INCORRECT, identi));
		}
	...
	public final Role update(final Role obj) throws java.lang.Exception {
		...
		if (roleTemp == null) {
			throw new Exception(ROLE_EXC_UPDATE + String.format(ROLE_EXC_OBJ_NOT_FOUND, obj.getId()));
		}
	 ...
	 *
	 * @param identi
	 *            Identificador del objeto a borrar
	...
	public final boolean delete(final int identi) throws java.lang.Exception {
		...
		// controles
		if (identi <= Role.ID_NULL) {
			throw new Exception(ROLE_EXC_DELETE + String.format(ROLE_EXC_ID_INCORRECT, identi));
		}

		// 1- Obtenemos el existente
		roleTemp = this.getByID(identi);
		if (roleTemp == null) {
			throw new Exception(ROLE_EXC_DELETE + String.format(ROLE_EXC_OBJ_NOT_FOUND, identi));
		}
